    ...
    public static void main (String [] args){
    	try {
           ...
           Usuario f = new Usuario();
            ...
            m.setMsg_conteudo("Se não existem muitos carrapatos dá para tirar com a mão mesmo ou uma pinça. E depois é só aplicar o remédio no período correto que elas não se instalam mais. Agora no verão tem que cuidar bastante e aplicar com mais frequência o remédio. Usamos sempre frontline ou um outro que é do laboratório bayer muito bom.");
            ...
            m1.setMsg_conteudo("Uma dica é lavar a picada com sabonete antibacteriano e água morna. Isso irá reduzir as chances de infectar. Depois de lavar a área, você pode também aplicar álcool para desinfetá-la.");
            m1.setMsg_data_cadastro(new Date());
            m1.setMsg_id_usu(f1);
            m1.setMsg_isPergunta(true);
            m1.setMsg_status(Constants.ATIVO);
            m1.setMsg_tipo_animal("Cachorro");
            m1.setMsg_titulo("Como cicatrizar as feridas de mordida de carrapato no cão?");
            
    		Mensagem m2 = new Mensagem();
   		 
            m2.setMsg_conteudo("Isso pode ter relação sim com o problema cardíaco...mas alguns cães roncam normalmente...sem que haja algum problema.Mas na duvida não deixe de consultar um veterinário!");
            m2.setMsg_data_cadastro(new Date());
            m2.setMsg_id_usu(f1);
            m2.setMsg_isPergunta(true);
            m2.setMsg_status(Constants.ATIVO);
            m2.setMsg_tipo_animal("Cachorro");
            m2.setMsg_titulo("Porque os cão roncam?");
            
        	Mensagem m3 = new Mensagem();
   		 
            m3.setMsg_conteudo("Se as lambidas se tornarem muito intensas e fugirem do controle, talvez seja o momento de distrair o gato e ensiná-lo a não lamber.");
            ...
            m4.setMsg_conteudo("Os gatos reagem à música de forma muito particular. Por vezes miam, outras vezes acalmam-se. Os gatos ouvem melhor do que os humanos, embora não captem os sons mais graves.");
            m4.setMsg_data_cadastro(new Date());
            m4.setMsg_id_usu(f1);
            m4.setMsg_isPergunta(true);
            m4.setMsg_status(Constants.ATIVO);
            m4.setMsg_tipo_animal("Gato");
            m4.setMsg_titulo("Os gatos gostam de ouvir músicas?");
